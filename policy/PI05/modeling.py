#!/usr/bin/env python
"""
PI05 PyTorch model — self-contained, no lerobot dependency.
Adapted from lerobot/src/lerobot/policies/pi05/modeling_pi05.py

Contains: PaliGemmaWithExpertModel, PI05Pytorch (the model you call for inference).
Does NOT contain PI05Policy (LeRobot training wrapper).
"""
...
def resize_with_pad_torch(images: torch.Tensor, height: int, width: int, mode: str = "bilinear") -> torch.Tensor:
    ...
    cur_height, cur_width = images.shape[2], images.shape[3]
    ratio = max(cur_width / width, cur_height / height)
    resized_height = int(cur_height / ratio)
    resized_width = int(cur_width / ratio)

    resized_images = F.interpolate(images, size=(resized_height, resized_width), mode=mode, align_corners=False if mode == "bilinear" else None)
...
def compute_layer_complete(layer_idx, inputs_embeds, attention_mask, position_ids, adarms_cond, paligemma, gemma_expert):
    ...
    att_output, _ = modeling_gemma.eager_attention_forward(paligemma.model.language_model.layers[layer_idx].self_attn, query_states, key_states, value_states, attention_mask, scaling)
...
class PaliGemmaWithExpertModel(nn.Module):
    ...
    def to_bfloat16_for_selected_params(self, precision: str = "bfloat16"):
    ...
    def embed_image(self, image: torch.Tensor):
        ...
        features = getattr(image_outputs, "pooler_output", image_outputs)
    ...
    def forward(self, attention_mask=None, position_ids=None, past_key_values=None, inputs_embeds=None, use_cache=None, adarms_cond=None):
        ...
        if inputs_embeds[1] is None:
            ...
            use_gradient_checkpointing = (
                (hasattr(self.gemma_expert.model, "gradient_checkpointing") and self.gemma_expert.model.gradient_checkpointing and self.training)
                or (hasattr(self, "gradient_checkpointing") and self.gradient_checkpointing and self.training)
            )
            for layer_idx in range(num_layers):
                if use_gradient_checkpointing:
                    inputs_embeds = torch.utils.checkpoint.checkpoint(
                        compute_layer_complete, layer_idx, inputs_embeds, attention_mask, position_ids, adarms_cond,
                        use_reentrant=False, preserve_rng_state=False,
                        paligemma=self.paligemma, gemma_expert=self.gemma_expert,
                    )
                else:
                    inputs_embeds = compute_layer_complete(layer_idx, inputs_embeds, attention_mask, position_ids, adarms_cond, paligemma=self.paligemma, gemma_expert=self.gemma_expert)
...
# ── PI05Pytorch — the model you call for inference ──────────────────────
...
class PI05Pytorch(nn.Module):
    ...
    def embed_suffix(self, noisy_actions, timestep):
        time_emb = create_sinusoidal_pos_embedding(timestep, self.action_in_proj.out_features, min_period=self.config.min_period, max_period=self.config.max_period, device=timestep.device)
    ...
    @torch.no_grad()
    def sample_actions(self, images, img_masks, tokens, masks, noise=None, num_steps=None, **kwargs) -> Tensor:
