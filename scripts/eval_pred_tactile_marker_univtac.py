#!/usr/bin/env python3
"""Offline eval: marker-based tactile encoder + future tactile prediction.
...
Example:
  ...
  bash scripts/run_insert_hdmi_tactile_marker_pred_eval.sh
"""
...
@torch.no_grad()
def run_single_sample(model, sample, device):
    states = sample["states"].unsqueeze(0).to(device)
    tactile_markers = sample["tactile_markers"].unsqueeze(0).to(device)
    target_tactile_markers = sample["target_tactile_markers"].unsqueeze(0).to(device)
    ...
    gt_tactiles = model.model.encode_target_tactile_trajectory(
        target_tactile_markers=target_tactile_markers.to(device=device)
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate marker-based tactile prediction for insert_hdmi WLA model."
    )
    parser.add_argument(
        "--deploy_config",
        default=str(DEFAULT_DEPLOY_CONFIG),
        help="UniVTAC deploy yaml, default: policy/wla/deploy_tactile_marker_predict_tactile.yml",
    )
    parser.add_argument(
        "--output_dir",
        default=str(UNIVTAC_ROOT / "eval_result/wla/insert_HDMI/tactile_marker_pred_vis"),
    )
    ...
    if deploy_cfg.get("tactile_input_type") != "marker":
        raise ValueError("Deploy config must set tactile_input_type: marker.")
    if not deploy_cfg.get("use_tactile_encoder", False):
        raise ValueError("Deploy config must use tactile encoder.")

    model, model_config, _, mantis_root, LeRobotEvalDataset, load_custom_lerobot_dataset = build_model(deploy_cfg)
    if model_config.get("tactile_input_type") != "marker":
        raise ValueError("Training config must have tactile_input_type: marker")
    ...
    for rank, dataset_idx in enumerate(sample_indices):
        ...
        if "tactile_markers" not in sample or "target_tactile_markers" not in sample:
            raise KeyError(
                "Dataset sample is missing tactile_markers/target_tactile_markers. "
                "Check tactile_marker_keys in training config and dataset_root_dir."
            )
        result = run_single_sample(model, sample, device)
